...
ANNkd_tree* globalKdTree;
ANNpointArray dataArray;

void initKdTree(double *data, int n, int dim, int bs, ANNsplitRule split)
{
	dataArray = annAllocPts(n, dim);
	for (int i = 0; i < n * dim; i++)
		dataArray[i / dim][i % dim] = data[i];
	globalKdTree = new ANNkd_tree(dataArray, n, dim, bs, split);
}

void annKdSearch(ANNpoint q, int k, ANNidxArray nnIdx, ANNdistArray dd, double eps)
{
	globalKdTree->annkSearch(q, k, nnIdx, dd, eps);
}

void annkPriSearch(ANNpoint q, int k, ANNidxArray nnIdx, ANNdistArray dd, double eps)
{
	globalKdTree->annkPriSearch(q, k, nnIdx, dd, eps);
}

void annkFRSearch(ANNpoint q, ANNdist sqRad, int k, ANNidxArray nnIdx, ANNdistArray dd, double eps)
{
